...
def mark_edge(e, marked_edges):
    normalized_edge = tuple(sorted(e))
    marked_edges[normalized_edge] += 1
    return marked_edges

def move(G, v, e_amin_jöttem, marked_edges):
    first_case = True
    normalized_e_amin_jöttem = tuple(sorted(e_amin_jöttem))
    G.remove_edge(normalized_e_amin_jöttem[0], normalized_e_amin_jöttem[1])
    
    for e in list(G.edges(v)):
        normalized_edge = tuple(sorted(e))
        if marked_edges[normalized_edge] != 0:
            first_case = False
    if list(G.edges(v)) == []:
        first_case = False

    if first_case:
        e = rnd.choice(list(G.edges(v)))
        normalized_edge = tuple(sorted(e))
        G.add_edge(normalized_e_amin_jöttem[0], normalized_e_amin_jöttem[1])
        for i in e:
            if i !=v:
                return i, e
    
    elif not first_case and marked_edges[normalized_e_amin_jöttem] == 1:
        G.add_edge(normalized_e_amin_jöttem[0], normalized_e_amin_jöttem[1])
        for i in e_amin_jöttem:
            if i !=v:
                return i, e_amin_jöttem
        
    elif not first_case and marked_edges[normalized_e_amin_jöttem] > 1:
        subdict = {}
        for e in list(G.edges(v)):
            normalized_edge = tuple(sorted(e))
            subdict[normalized_edge] = marked_edges[normalized_edge]
        e = min(subdict, key=subdict.get)
        G.add_edge(normalized_e_amin_jöttem[0], normalized_e_amin_jöttem[1])
        for i in e:
            if i !=v:
                return i, e
...
def Trémaux(T, start, end):
    done= False
    v = start
    marked_edges = {tuple(sorted(e)): 0 for e in list(T.edges())}
    e = list(T.edges(v))[0]
    marked_edges = mark_edge(e, marked_edges)
    v = e[1]
    e_amin_jöttem = e
    lista = [start]
    while not done:
        lista.append(v)
        v, e_amin_jöttem = move(T, v, e_amin_jöttem, marked_edges)
        marked_edges = mark_edge(e_amin_jöttem, marked_edges)      
        if v == end:
            lista.append(v)
            done = True
    return lista
